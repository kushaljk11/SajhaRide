...
<%
    ...
    List<Vehicle> exploreVehicles = (List<Vehicle>) request.getAttribute("exploreVehicles");
...
<div class="flex h-full">
    ...
    <div class="flex min-w-0 flex-1 flex-col">
        ...
        <main class="flex-1 overflow-y-auto p-6 lg:p-8">
            ...
            <section class="mt-6">
                ...
                <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <% for (Vehicle vehicle : vehicles) { 
                            String imgPath = vehicle.getImagePath();
                    ...
                    <article class="group overflow-hidden rounded-2xl bg-gray-50 ring-1 ring-gray-200 transition hover:-translate-y-0.5 hover:shadow-sm">
                        <div class="h-32 overflow-hidden">
                            <img src="${pageContext.request.contextPath}/<%= imgPath %>" alt="<%= vehicle.getVehicleName() %>" class="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
                        </div>
                        <div class="p-4">
            ...
            <section class="mt-6">
                <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                    <h2 class="text-xl font-semibold text-gray-900">Explore Available Rides</h2>
                    <a href="${pageContext.request.contextPath}/vehicles/list" class="text-sm font-semibold text-red-800 hover:text-red-900">View All</a>
                </div>
                <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    <% if (exploreVehicles == null || exploreVehicles.isEmpty()) { %>
                    <article class="rounded-2xl bg-white p-5 ring-1 ring-gray-200 sm:col-span-2 xl:col-span-4">
                        <p class="text-sm text-gray-600">No vehicles available to explore right now.</p>
                    </article>
                    <% } else {
                        for (int i = 0; i < exploreVehicles.size() && i < 4; i++) {
                            Vehicle v = exploreVehicles.get(i);
                            // Don't show the renter's own vehicles in explore
                            if (v.getOwnerId() == dashboardUser.getUserId()) continue;
                    ...
                    %>
                    <article class="group overflow-hidden rounded-2xl bg-white ring-1 ring-gray-200 transition hover:-translate-y-0.5 hover:shadow-sm">
                        ...
                        <div class="p-4">
                            <h3 class="font-semibold text-gray-900"><%= v.getVehicleName() %></h3>
            ...
            <section class="mt-6">
                <article class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-200">
                    ...
                    <div class="mt-5 flex gap-3">
                        <button type="button" class="rounded-xl bg-gray-100 px-6 py-2.5 text-sm font-semibold text-gray-700 transition hover:bg-gray-200">Read FAQs</button>
                        <a href="${pageContext.request.contextPath}/contact" class="inline-flex items-center justify-center rounded-xl bg-red-800 px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-red-900">Contact Support</a>
                    </div>
                </article>
            </section>
        </main>

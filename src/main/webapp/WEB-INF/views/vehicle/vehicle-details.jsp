  ...
  <% } %>
    ...
    <section class="grid gap-6 lg:grid-cols-[1.3fr_0.9fr]">
      ...
      <aside class="space-y-6">
        ...
        <section class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200"><h2 class="text-xl font-semibold text-slate-900">Actions</h2><p class="mt-2 text-sm leading-6 text-slate-600">Book this vehicle or return to the explore page.</p><div class="mt-5 flex flex-col gap-3"><a href="<%= request.getContextPath() %>/booking/request?vehicleId=<%= vehicle.getVehicleId() %>" class="inline-flex items-center justify-center rounded-xl bg-red-800 px-5 py-3 text-sm font-semibold text-white transition hover:bg-red-900">Book Now</a><a href="<%= request.getContextPath() %>/user/explore" class="inline-flex items-center justify-center rounded-xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Back to Explore</a></div></section>
      </aside>
    </section>
    <% } %>
      </main>
